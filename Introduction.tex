...
\begin{figure}[h]
\centering
\includegraphics[scale=0.35]{thomson.png}
...
The key limitation in detecting PMFs will be the experimental senstivity, measured in noise ($\mu$K). To improve sensitivity one must decrease the noise, which is done by adding more detectors. The noise level and the number of detectors obey the relation: $$ noise \propto ({\sqrt{n_{detectors}})^{-1}$$ Each stage of CMB experiments adds a factor of 10 more detectors, bringing the noise level down by a factor of $\sqrt{10}$.

\begin{figure}
\centering
\includegraphics[scale=0.75]{images/experiments.png} 
\caption{Plot of approximate raw experimental sensitivity of CMB experiments vs time in years. The Red lines indicate space-based experiments. Of note is PLANCK whose sensitivity falls in the middle-range of the stage-2 CMB experiments. The dotted cyan line represents the sensitivity of CMB-S3, which we expect to improve by up to an order of magnitude over Stage 2. CMB-S4 is represented by the dotted purple line, which is set to improve by a further order of magnitude over S3.}
\end{figure}
...
Most of it's written up: include pictures for each section:
Some to consider:
CMB power spectrum TT,EE,BB
Galactic dynamo?
Faraday rotation?
Picture of SPT-3g?
...
Consult your table, name some experiments
BICEP2, 2
PLANCK, space
ADV. ACTPOL, 3
Simons Array, 3
LiteBird?, ?
Pixie?, 4
I need to know sensitivities : check the stuff christian gave you.

CMB-S4
Nothing set in stone just yet.
To attain CMB-S4 science goals, of order 500,000 detectors and a four year survey are neeeded. Attains 1 muK sensitivity over 70% of the sky. New, Northern telescope is desireable for 100% sky coverage.
\fi
...
SPT-pol & \multicolumn{1}{c|}{2} & \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{1536} & \multicolumn{1}{c|}{14} & \multicolumn{1}{c}{20} \\
SPT-3G & \multicolumn{1}{c|}{3} & \multicolumn{1}{c|}{15234} & \multicolumn{1}{c|}{2500} & \multicolumn{1}{c|}{3.4} & \multicolumn{1}{c}{4.8} \\
 &  &  &  & 
\end{tabular}
\end{table}
